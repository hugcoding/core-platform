...
COPY (
  SELECT id, file_id, decision, proposal_category_code,
         proposal_document_family_code, corrected_category_code,
         corrected_document_family_code, proposed_target_path,
         proposed_target_path_raw, created_at
  FROM public.document_review_events
  WHERE channel = 'workset_portal'
...
def main(argv=None) -> int:
    ...
    payload = {
        "schema_version": "review-learning-candidates-v1",
    ...
    lines = ["# SCRUM-98 kandidaatregels uit portalbeoordelingen", "",
             ...
             f"- Minimum support: **{args.minimum_support}**", f"- Kandidaatregels: **{len(candidates)}**",
