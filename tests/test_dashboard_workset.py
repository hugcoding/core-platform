...
class DashboardWorksetTests(unittest.TestCase):
    ...
    def test_original_path_is_clickable_with_compact_copy_icon(self):
        ...
        self.assertIn('class="open-original-folder"', script)
        self.assertIn('aria-label="Map openen"', script)
        self.assertIn("function folderOpenUrl", script)
        self.assertIn('data-source-file="${wsEsc(doc.file_id)}"', script)
        self.assertIn("source.dataset.sourceFile", script)
