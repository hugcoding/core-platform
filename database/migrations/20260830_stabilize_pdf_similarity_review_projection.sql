-- SCRUM-115: keep a human PDF-similarity decision effective while reviewed
-- redundant copies leave the active set during controlled quarantine.
BEGIN;
...
LEFT JOIN public.v_latest_pdf_content_similarity_review r
 ...
 -- Removing a previously reviewed redundant file is expected during a
 -- partially completed quarantine batch. Keep that append-only decision.
...
JOIN public.v_latest_pdf_content_similarity_evidence leader_evidence
  ON leader_evidence.file_id = leader.id
...
JOIN public.v_latest_pdf_content_similarity_evidence redundant_evidence
  ON redundant_evidence.file_id = redundant.id
WHERE r.action = 'selected_leader';

COMMENT ON VIEW public.v_pdf_content_similarity_groups IS
  'Advisory PDF groups. Existing reviews remain effective for an active subset after controlled quarantine.';
COMMENT ON VIEW public.v_pdf_content_similarity_quarantine_handoff IS
  'Read-only, revalidated handoff. Physical quarantine still requires a separately approved execution batch.';
