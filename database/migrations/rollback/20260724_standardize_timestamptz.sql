...
FROM public.files
WHERE updated_at >= (now() AT TIME ZONE 'UTC') - interval '1 hour';

CREATE VIEW public.v_files_over_time_extended AS
WITH per_day AS (
    SELECT date(created_at) AS file_date, count(*) AS files_created,
           count(*)::numeric / 24.0 AS avg_files_per_hour_that_day
    FROM public.files GROUP BY date(created_at)
), global_stats AS (
    SELECT count(*) AS total_files, min(created_at) AS first_file,
           max(created_at) AS last_file,
           extract(epoch FROM (max(created_at) - min(created_at))) / 3600 AS hours_span
    FROM public.files
)
SELECT p.*, g.total_files, g.first_file, g.last_file, g.hours_span,
       CASE WHEN g.hours_span < 1 THEN NULL
            ELSE g.total_files::numeric / g.hours_span END AS avg_files_per_hour_global
FROM per_day p CROSS JOIN global_stats g ORDER BY p.file_date;
...
SELECT id, type, status, started_at, finished_at, files_discovered,
       ...
       round(jobs_processed::numeric / NULLIF(jobs_enqueued, 0)::numeric * 100, 2)
           AS progress_percent
FROM public.scan_sessions ORDER BY started_at DESC;
...
FROM public.files f JOIN public.folders fo ON fo.id = f.folder_id
