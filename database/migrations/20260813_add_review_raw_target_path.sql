-- SCRUM-98: retain raw human path input beside its safe normalized value.
BEGIN;
ALTER TABLE public.document_review_events
    ADD COLUMN IF NOT EXISTS proposed_target_path_raw text;
ALTER TABLE public.document_review_events
    DROP CONSTRAINT IF EXISTS document_review_events_proposed_path_raw_check;
ALTER TABLE public.document_review_events
    ADD CONSTRAINT document_review_events_proposed_path_raw_check
        CHECK (proposed_target_path_raw IS NULL OR length(proposed_target_path_raw) BETWEEN 1 AND 500);
