...
async function requestOcr(fileId,actions){const message=actions.querySelector('.ai-job-message');message.textContent='OCR-aanvraag toevoegen...';actions.querySelectorAll('button').forEach(button=>button.disabled=true);try{const response=await fetch('/api/v1/workset/ocr-jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({file_id:Number(fileId),idempotency_key:reviewId()})});const data=await response.json();if(!response.ok)throw Error(data.detail||response.status);message.textContent='OCR staat in de wachtrij';await refreshAiQueue()}catch(error){message.textContent=`OCR-aanvraag mislukt: ${error.message}`;actions.querySelectorAll('button').forEach(button=>button.disabled=false)}}
