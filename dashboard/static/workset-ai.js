...
function aiAction(job,doc){if(!job)return`<button type="button" class="request-ai" data-ai-file="${doc.file_id}">Vraag AI-voorstel aan</button><span class="ai-job-message"></span>`;const ocrRecommended=isOcrAdvice(job),knownOcr=job.reason==='ocr_required_from_existing_evidence',retry=['failed','cancelled'].includes(job.status)||job.status==='abstained'&&!ocrRecommended?`<button type="button" class="request-ai" data-ai-file="${doc.file_id}">Opnieuw aanvragen</button>`:'',ocr=ocrRecommended?`<span class="ai-ocr-advice" role="status"><strong>${knownOcr?'OCR vereist — reeds vastgesteld':'OCR aanbevolen'}</strong><small>${knownOcr?'CORE hergebruikte actuele extractie-evidence voor dezelfde bestandsinhoud.':'CORE vond geen herkenbare tekst.'} Laat eerst OCR uitvoeren en vraag daarna opnieuw een AI-voorstel aan.</small></span>`:'',accept=job.status==='ready'
