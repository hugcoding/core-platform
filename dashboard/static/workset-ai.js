...
function aiReasonInDutch(reason){const known={waiting_for_cpu:'Wacht op lagere CPU-belasting',waiting_for_memory:'Wacht op voldoende vrij geheugen',core_pipeline_priority:'CORE-verwerking heeft voorrang',ai_worker_busy:'De lokale AI-worker is bezig',provider_unavailable:'De lokale AI-provider is niet bereikbaar',ocr_recommended_no_extractable_text:'Geen herkenbare tekst gevonden; OCR wordt aanbevolen',no_extractable_text:'Geen uitleesbare tekst gevonden'};return known[reason]||reason||''}
async function refreshAiQueue(){try{const response=await fetch('/api/v1/workset/ai-jobs',{cache:'no-store'});if(!response.ok)throw Error(response.status);const data=await response.json();aiQueue.summary=data.summary;aiQueue.jobs=new Map(data.jobs.map(job=>[Number(job.file_id),job]));aiQueue.loaded=true;renderAiBell(data.jobs);decorateAiActions()}catch(error){const bell=document.querySelector('.ai-notification');if(bell)bell.title='AI-wachtrij niet beschikbaar'}}
...
function renderAiBell(jobs){const bell=ensureAiBell(),ready=jobs.filter(job=>(job.status==='ready'&&job.awaiting_human_review)||(job.status==='abstained'&&job.reason==='ocr_recommended_no_extractable_text'));bell.querySelector('.ai-bell b').textContent=ready.length;bell.classList.toggle('has-ready',ready.length>0);bell.querySelector('.ai-ready-list div').innerHTML=ready.length?ready.map(job=>`<button type="button" data-ai-ready-file="${job.file_id}" data-filename="${wsEsc(job.filename)}"><span>${wsEsc(job.filename)}</span><small>${wsEsc(job.reason==='ocr_recommended_no_extractable_text'?'OCR aanbevolen':aiStatusLabel(job.status))}</small></button>`).join(''):'<p>Geen nieuwe voorstellen of OCR-adviezen.</p>'}
...
function aiAction(job,doc){if(!job)return`<button type="button" class="request-ai" data-ai-file="${doc.file_id}">Vraag AI-voorstel aan</button><span class="ai-job-message"></span>`;const ocrRecommended=job.status==='abstained'&&job.reason==='ocr_recommended_no_extractable_text',retry=['failed','cancelled'].includes(job.status)||job.status==='abstained'&&!ocrRecommended?`<button type="button" class="request-ai" data-ai-file="${doc.file_id}">Opnieuw aanvragen</button>`:'',ocr=ocrRecommended?`<span class="ai-ocr-advice" role="status"><strong>OCR aanbevolen</strong><small>CORE vond geen herkenbare tekst. Laat eerst OCR uitvoeren en vraag daarna opnieuw een AI-voorstel aan.</small></span>`:'',accept=job.status==='ready'
