...
function aiBellEntry(job){if(job.workset_available===false)return null;const ocrJob=ocrQueue.jobs.get(Number(job.file_id));if(job.status==='ready'&&job.awaiting_human_review)return{...job,notification_label:'Voorstel gereed'};if(!isOcrAdvice(job))return null;if(!ocrJob)return{...job,notification_label:'OCR aanbevolen'};if(ocrJob.status==='ready')return{...job,notification_label:'OCR gereed'};if(ocrJob.status==='failed')return{...job,notification_label:'OCR mislukt'};return null}
function renderAiBell(jobs){const bell=ensureAiBell(),ready=latestAiJobsByFile(jobs).map(aiBellEntry).filter(Boolean);bell.querySelector('.ai-bell b').textContent=ready.length;bell.classList.toggle('has-ready',ready.length>0);bell.querySelector('.ai-ready-list div').innerHTML=ready.length?ready.map(job=>`<button type="button" data-ai-ready-file="${job.file_id}" data-filename="${wsEsc(job.filename)}"><span>${wsEsc(job.filename)}</span><small>${wsEsc(job.notification_label)}</small></button>`).join(''):'<p>Geen nieuwe voorstellen of OCR-acties.</p>'}
