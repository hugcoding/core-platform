...
function params(){return new URLSearchParams({status:ws('worksetStatus').value,review_state:ws('worksetReview').value,review_decision:ws('worksetDecision').value,extension:ws('worksetExtension').value,family:ws('worksetFamily').value,search:ws('worksetSearch').value.trim(),limit:state.limit,offset:state.offset})}
function renderReviewStats(){const s=state.reviewSummary;ws('worksetReviewStats').innerHTML=[['Open',s.pending],['Beoordeeld',s.reviewed],['Akkoord',s.accepted],['Later',s.needs_review],['Niet akkoord',s.rejected],['Overgeslagen',s.passed]].map(([label,value])=>`<span class="review-stat">${label}<b>${wsNf.format(value||0)}</b></span>`).join('')}
