...
const state={offset:0,limit:50,loading:false,documents:[],families:[],reviewEnabled:false,privacyReviewEnabled:false,nominationEnabled:false,llmEnabled:false,filteredTotal:0,reviewSummary:{},nominationSummary:{archive:0,deletion:0},worksetSummary:{},taxonomy:{categories:[],families:[]}};
...
function nominationActions(doc){
  ...
  return `<div class="nomination-actions" data-file-id="${doc.file_id}"><button type="button" class="nomination-button${archive?' active':''}" data-nomination-type="archive" data-nomination-action="${archive?'withdrawn':'nominated'}" title="${archive?'Archiefnominatie intrekken':'Nomineren voor archiefreview'}"><span aria-hidden="true">&#128230;</span>${archive?'Archief genomineerd':'Archiveren'}</button><button type="button" class="nomination-button deletion${deletion?' active':''}" data-nomination-type="deletion" data-nomination-action="${deletion?'withdrawn':'nominated'}" title="${deletion?'Verwijdernominatie intrekken':'Nomineren voor toekomstige verwijderreview'}"><span aria-hidden="true">&#128465;</span>${deletion?'Verwijderreview '+wsDt(deletion.review_at):'Nomineren voor verwijderen'}</button><span class="nomination-message" aria-live="polite"></span></div>`;
}
...
async function submitNomination(button){
  ...
  let reason=action==='withdrawn'?`Handmatig ingetrokken: ${label}`:globalThis.prompt(`Waarom wil je dit document nomineren voor ${label}?`,'Niet meer nodig in de huidige vorm');
  if(reason===null)return;reason=reason.trim();if(!reason)return;
  ...
  try{const response=await fetch('/api/v1/workset/nominations',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({file_id:Number(panel.dataset.fileId),idempotency_key:reviewId(),nomination_type:type,action,reason})});if(!response.ok)throw Error(response.status);const data=await response.json();message.textContent=action==='nominated'?`${label} genomineerd; review ${wsDt(data.review_at)}.`:'Nominatie ingetrokken.';setTimeout(()=>loadWorkset(true),500)}catch(error){message.textContent='Nominatie kon niet worden opgeslagen.';panel.querySelectorAll('button').forEach(item=>item.disabled=false)}
}
