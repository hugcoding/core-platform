...
function decorateFilenameProposals() {
  document.querySelectorAll('.review-panel').forEach(panel => {
    ...
    const savedFilename = doc?.latest_proposed_filename_raw || doc?.latest_proposed_filename || '';
    const label = document.createElement('label'); label.innerHTML = `Nieuwe bestandsnaam<input class="proposed-filename" maxlength="255" value="${wsEsc(savedFilename)}" placeholder="${wsEsc(doc?.filename || 'Nieuwe naam')}"><small>De huidige extensie blijft behouden.</small>`;
...
function documentCard(doc) {
  ...
  const proposal = doc.target_proposal, classification = doc.classification_status === 'accepted' ? `${wsEsc(proposal?.category_label || doc.effective_category || doc.category)} · ${wsEsc(proposal?.folder_label || doc.effective_document_family || doc.document_family || 'familie onbekend')}` : 'Nog niet inhoudelijk beoordeeld', path = doc.smb_path || doc.path, pathSource = doc.current_physical_path ? 'Geverifieerd huidig pad' : 'Geregistreerd pad; geen geverifieerde migratielocatie'; const target = proposal ? `<div class="target-proposal"><span><i class="source-badge">CORE-voorstel</i>${wsEsc(proposal.proposal_confidence)}</span><code>${wsEsc(proposal.suggested_target_path)}</code>${sourceContextHint(proposal)}</div>` : ''; const refinements = [['Nieuwe categorie', doc.latest_proposed_category_label], ['Nieuwe familie', doc.latest_proposed_family_label], ['Nieuwe bestandsnaam', doc.latest_proposed_filename], ['Nieuw doelpad', doc.latest_proposed_target_path]].filter(item => item[1]); const refinementSummary = refinements.length ? `<div class="human-refinement-summary"><b>Jouw aanvullende voorstellen</b>${refinements.map(item => `<span>${wsEsc(item[0])}: <strong>${wsEsc(item[1])}</strong></span>`).join('')}</div>` : ''; const latest = doc.latest_review_decision ? `<div class="latest-review"><i class="source-badge">Menselijk oordeel</i><b>${wsEsc(decisionLabels[doc.latest_review_decision] || doc.latest_review_decision)}</b> · ${wsDt(doc.latest_review_at)} <button class="history-button" data-history-file="${doc.file_id}">Historie</button>${refinementSummary}<div class="review-history" hidden></div></div>` : ''; let review = ''; if (state.reviewEnabled && proposal) {
    ...
    const family = useSimilar
      ...
      : proposal.document_family_code; const learnedHint = useSimilar
        ...
        : ''; const proposedPathValue = doc.is_deletion_quarantined && String(doc.latest_proposed_target_path || '').toLowerCase().startsWith('/volume1/data/persoonlijk/quarantaine/') ? '' : (doc.latest_proposed_target_path || ''); review = `${learnedHint}<div class="review-panel" data-file-id="${doc.file_id}" data-previous-decision="${wsEsc(doc.latest_review_decision || '')}"><label>Categorie<select class="review-category">${categoryOptions(category)}</select></label><label>Familie<select class="review-family">${compactFamilyOptions(doc, family, category)}</select></label><button type="button" class="more-families">Meer…</button><div class="family-browser" hidden><label>Zoek familie<input class="family-search" type="search" placeholder="Zoeken"></label><label>Alle passende families<select class="all-families" size="6">${allFamilyOptions(category, '', family)}</select></label></div><label>Notitie<input class="review-note" maxlength="2000" value="${wsEsc(doc.latest_review_notes || '')}" placeholder="Toelichting voor CORE"></label><details class="new-proposal"><summary>Nieuwe categorie, familie of doelpad voorstellen</summary><label>Nieuwe categorie<input class="proposed-category" maxlength="120" value="${wsEsc(doc.latest_proposed_category_label || '')}" placeholder="Bijv. Wonen"></label><label>Nieuwe familie<input class="proposed-family" maxlength="120" value="${wsEsc(doc.latest_proposed_family_label || '')}" placeholder="Bijv. VvE-document"></label><label>Nieuw doelpad<input class="proposed-path" maxlength="500" value="${wsEsc(proposedPathValue)}" placeholder="Bijv. Wonen/VvE/Eksterlaan"></label><small>Wordt apart beoordeeld en wijzigt de CORE-regels niet automatisch.</small></details><div class="review-actions"><button data-decision="accepted" class="accept">Classificatie akkoord</button><button data-decision="needs_review">Later</button><button data-decision="rejected" class="reject">Niet akkoord</button><button data-decision="passed">Overslaan</button></div><span class="review-message" aria-live="polite"></span></div>`
...
</div><code data-source-file="${wsEsc(doc.file_id)}" title="${wsEsc(pathSource)}">${wsEsc(doc.path)}</code><small class="path-source">${wsEsc(pathSource)}</small>${inherited}${target}${latest}${doc.is_similarity_redundant?'':privacyCard(doc)}${review}</div><button class="copy-path" type="button" data-path="${wsEsc(path)}">Kopieer SMB-pad</button></article>`
}
...
function decorateNominationCards() {
  ...
  document.querySelectorAll('.document-card').forEach((card, index) => {
    const doc = state.documents[index]; if (!doc || doc.is_similarity_redundant || card.querySelector('.nomination-actions')) return;
...
async function toggleHistory(button) { const container = button.parentElement.querySelector('.review-history'); if (!container.hidden) { container.hidden = true; return } container.hidden = false; container.innerHTML = 'Historie laden…'; try { const response = await fetch(`/api/v1/workset/${button.dataset.historyFile}/reviews`, { cache: 'no-store' }); if (!response.ok) throw Error(response.status); const data = await response.json(); container.innerHTML = data.events.length ? data.events.map(event => { const refinements = [['Nieuwe categorie', event.proposed_category_label], ['Nieuwe familie', event.proposed_family_label], ['Nieuwe bestandsnaam', event.proposed_filename], ['Nieuw doelpad', event.proposed_target_path]].filter(item => item[1]).map(item => `<br>${wsEsc(item[0])}: <strong>${wsEsc(item[1])}</strong>`).join(''); return `<div class="history-event"><b>${wsEsc(decisionLabels[event.decision] || event.decision)}</b> · ${wsDt(event.created_at)} · ${wsEsc(event.reviewer)}<br>Categorie: ${wsEsc(event.corrected_category_code || event.proposal_category_code || 'onbekend')} · Familie: ${wsEsc(event.corrected_document_family_code || event.proposal_document_family_code || 'onbekend')}${event.review_type === 'privacy_classification' ? ` · Privacy: ${wsEsc(event.corrected_privacy_classification || event.proposal_privacy_classification)}` : ''}${refinements}${event.review_notes ? `<code>${wsEsc(event.review_notes)}</code>` : ''}</div>` }).join('') : 'Geen eerdere beoordelingen' } catch (error) { container.innerHTML = 'Historie laden mislukt' } }
