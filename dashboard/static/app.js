...
async function refresh(){if(refreshing)return;refreshing=true;const button=$('refresh');button.disabled=true;button.classList.add('loading');try{const r=await fetch('/api/v1/overview',{cache:'no-store'});if(!r.ok)throw Error(r.status);const d=await r.json();
 ...
 }catch(e){document.body.dataset.state='degraded';$('overall').textContent='Dashboard offline';$('summary').textContent='Vernieuwen mislukt · live API niet bereikbaar.'}finally{refreshing=false;button.disabled=false;button.classList.remove('loading')}}
$('refresh').addEventListener('click',refresh);setInterval(()=>{$('clock').textContent=new Date().toLocaleTimeString('nl-NL',{hour:'2-digit',minute:'2-digit',second:'2-digit'})},1000);refresh();setInterval(refresh,10000);
