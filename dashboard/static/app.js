...
async function refresh(){try{const r=await fetch('/api/v1/overview',{cache:'no-store'});if(!r.ok)throw Error(r.status);const d=await r.json();
 ...
 $('queues').innerHTML=[['Polling',d.metrics.polling_queue],['Realtime',d.metrics.realtime_queue],['DLQ',d.metrics.dlq],['AI wachtend',d.metrics.ai_pending],['AI bezig',d.metrics.ai_running],['AI gereed',d.metrics.ai_ready],['AI mislukt',d.metrics.ai_failed],['Dirty roots',d.metrics.dirty_roots],['Lege bestanden',d.metrics.empty_files],['Gewijzigd 24u',d.metrics.changed_24h]].map(([k,v])=>`<div><span>${k}</span><b>${nf.format(v??0)}</b></div>`).join('');
